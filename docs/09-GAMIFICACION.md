...
El XP **no se convierte** 1:1 en Mazorcas Doradas ni se gasta. Las MD se acreditan con montos declarados en `mazorcaRewards` (emisión baja + topes diarios) y, además, el XP **apalanca** el bono semanal del balanced scorecard (hasta ×1.12) según rol y maestría — ver `16-MAZORCAS-DORADAS.md` y `26-ECONOMIA-MD-SCORECARD.md`. Completar un módulo de microlearning con sesión activa guarda además `campus_progress` bajo `course_slug = microlearning-caua`.
...
- Cacao Gotchi / Sembrar: crecimiento horario, modelo araucano (FEAR 5 · FTA 2 · FSA 13), catálogo Ecoyuma, disputa DO Arauca vs Orinoquía (sin DO inventada) y cosecha con MD reforzadas.

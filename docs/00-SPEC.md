...
| D17 | Dominio propio | **Decidido: `cacaocolab.org`** (comprado 2026-07-31) | Wiring en código + checklist DNS/Vercel/Resend/Supabase en `docs/24-DOMINIO-CACAOCOLAB-ORG.md`. `*.vercel.app` sigue como alias de deploy. |
...
| D26 | Campus registrado | **Solo magic link / OTP por email** para learners y builders (sin Google/Apple) | `/cuenta/entrar` + `/unete`. OAuth retirado hasta existir cuentas de desarrollador. Ver `27-MAGIC-LINK-AUTH.md`. |
...
| D34 | Montos de MD declarados | Las recompensas viven en `mazorcaRewards`, no se calculan desde el XP | Emisión sostenible: 6 MD por micro/misión Masters; 24 MD por curso Architect/Chocolatier; 5/16 Benevolo; cuidado 2 (tope 10/día); cosecha Sembrar +12 al abrir lote y +28 al cerrar fermentación; learning tope 20 MD/día. El XP no se gasta ni se cambia 1:1; solo apalanca el bono scorecard (D41). |
| D41 | Economía MD scorecard | Fintech gamificada: comprar packs, canjear cursos/aceleraciones, bono BSC por productividad propia | Sin multinivel ni downline. Packs no suman a `lifetime_earned`. XP leverage ≤1.12. Pool semanal 16 MD con techos bajos por rango. Ver `26-ECONOMIA-MD-SCORECARD.md`. |
...
| D39 | Chocolate Benevolo | Marca acelerada Bars.: chocolate de leche con marañón sugar free; Trinitario Arauca · Cundinamarca · Meta (FEAR 5 Quara ancla × Zurych). Manifiesto: antojo, no lección. Propósito: benevolencia del oficio. | Preventa por WhatsApp; hub `/benevolo`; track `/campus/benevolo`. Sin checkout/stock inventado ni medalla COEX atribuida. |

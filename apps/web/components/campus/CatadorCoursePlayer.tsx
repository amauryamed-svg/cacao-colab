...
const TIPS = [...catadorCompanionTips, ...catadorCompanionTipsShared]

function isCourseDone(state: RigorState) {
  return (
    Boolean(state.diplomaCode) ||
    catadorMissions.every((mission) => state.completed.includes(mission.slug))
  )
}
...
function ensureDiploma(state: RigorState, learnerName: string): RigorState {
  ...
  const g = gradeFromFirstTry(ft, catadorMissions.length)
  const payload: DiplomaPayload = {
    v: 1,
    course: CATADOR_COURSE_SLUG,
...
export default function CatadorCoursePlayer({
  learnerName,
  initialState,
}: {
  learnerName: string
  ...
  const boot = useMemo(() => {
    ...
    const firstOpen = catadorMissions.findIndex(
      (mission) => !withDiploma.completed.includes(mission.slug),
    )
    return {
      progress: withDiploma,
      missionIndex: done ? catadorMissions.length - 1 : firstOpen === -1 ? 0 : firstOpen,
  ...
  const mission = catadorMissions[missionIndex]
  ...
  const grade = gradeFromFirstTry(firstTryCount, catadorMissions.length)
  const gradeHint = nextGradeHint(firstTryCount, catadorMissions.length)
      ...
      (phase === "learn"
        ...
        : phase === "quiz"
          ...
          : phase === "course-complete"
            ? 1
            : 0)) /
      catadorMissions.length) *
      100,
  )
  const nextMissionUnlocked =
    missionIndex === 0 || progress.completed.includes(catadorMissions[missionIndex - 1]?.slug)

  const diplomaUrl = useMemo(() => {
    if (!progress.diplomaCode) return null
    return `${siteOrigin()}/credencial/catador-cacao/${progress.diplomaCode}`
  }, [progress.diplomaCode])

  const masteryMissions = catadorMissions.map((item) => {
  ...
  function persist(next: RigorState, complete = false) {
    try {
      localStorage.setItem("catador_progress_v1", JSON.stringify(next))
    } catch {
      // remoto
    }
    setSyncStatus("saving")
    void saveCatadorProgress(next, next.xp, complete || isCourseDone(next)).then((result) => {
  ...
  function finishAfterCorrect(score: { attempts: number; firstTry: boolean }) {
    ...
    const allDone = next.completed.length >= catadorMissions.length
  ...
  function continueCampaign() {
    ...
    const nextIndex = Math.min(missionIndex + 1, catadorMissions.length - 1)
  ...
  function selectMission(index: number) {
    const unlocked = index === 0 || progress.completed.includes(catadorMissions[index - 1].slug)
    if (!unlocked) return
    if (isCourseDone(progress) && index === catadorMissions.length - 1) {
    ...
    setQuizPassed(false)
    setPendingScore(null)
    setPhase(progress.completed.includes(catadorMissions[index].slug) ? "mission-complete" : "learn")
  }
    ...
    <div className="architect-player chocolatier-player">
      ...
      <div className="architect-layout">
        <aside className="architect-map">
          <p className="eyebrow text-[#FF6A3D]">Certificación · Master Catador</p>
          <h1>
            Master
            <br />
            Catador
          ...
          <p className="architect-welcome">
            {learnerName} · {completedCount}/6 · primer intento {firstTryCount}/6 · {catadorTotalXp} XP
          </p>
          ...
          <div className="architect-mission-rail">
            {catadorMissions.map((item, index) => {
              const done = progress.completed.includes(item.slug)
              const unlocked =
                index === 0 || progress.completed.includes(catadorMissions[index - 1].slug)
              const score = progress.scores[item.slug]
              return (
                <button
                  key={item.slug}
          ...
          <CampusSourcesPanel compact title="CoEx · rueda Colab · set catación" />
          ...
          <Link href="/rd/set-catacion" className="chocolatier-sister-link">
            Set Catación Colombia 10 →
          </Link>
          ...
          {!nextMissionUnlocked ? (
            ...
            <article className="architect-lesson-card" key={`${mission.slug}-${stepIndex}`}>
              ...
              <div className="architect-field-action">
                <span>◎ Lente CoEx · Rueda Colab · práctica</span>
              ...
              <p className="chocolatier-coex-note">
                <strong>Callebaut:</strong> {mission.callebautBridge}
              </p>
            ...
            <article className="architect-lesson-card">
              ...
              {quizPassed && pendingScore && (
                ...
                >
                  {missionIndex >= catadorMissions.length - 1
                    ? "Ver calificación y diploma →"
                    : "Continuar →"}
                </button>
              )}

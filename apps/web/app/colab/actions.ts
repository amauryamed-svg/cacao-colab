...
function sanitizeDiplomaUrl(raw?: string | null): string | null {
  if (!raw) return null
...
export async function loadForumFeed(limit = 40): Promise<ForumFeedItem[]> {
  ...
  // any: tipado generado aún no incluye diploma_url
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  if (error || !data) {
    return {
      ...
      error: error?.message?.includes("colab_forum")
        ? "Aplica la migración del muro en Supabase"
...
/** Semilla si el muro está vacío (best-effort). */
export async function ensureForumSeed(): Promise<void> {
  try {
    ...
    await supabase.from("colab_forum_posts").insert({
      profile_id: user.id,
      kind: "sync",
      title: "Bienvenida al muro de la comunidad",
      body: "Aquí compartimos avances de maestría, diplomas digitales y sincronicidades. Reacciona con 🍫 y practica lo aprendido en Sembrar. El diploma certifica rigor — la comunidad lo celebra.",
      course_slug: null,
      grade: null,
    })
  } catch {
